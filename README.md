...
## Результаты (метрики качества)

По результатам запуска офлайн-пайплайна в ноутбуке `recommendations.ipynb`:
...
Для холодных пользователей (fallback на топ популярных): **Recall_gen (top_popular)** = 5.51%.
